function [ ] = generate_permittivity_map_function_short( numberID )
...
%   Grabs information from .in file using format that was specified
...
line_data = textscan(fid,'%s%s','Delimiter',':','headerlines',38);
fid = fclose(fid);
...
if exist('box', 'var')
    for i=1:size(box,1)
        ...
        permittivity = sscanf(material{i},'m%d');
        box_material = double(g(permittivity));
        ...
        map = insertShape(map, 'FilledRectangle', ...            
            ...
            'Color',...
            [box_material box_material box_material],...
            'Opacity', 1 );         
...
if exist('cylinder', 'var')
    ...
    for i=1:size(cylinder,1)
        ...
        permittivity = sscanf(material{i},'m%d');
        cylinder_material = double(g(permittivity));
